...
// Change what interpolation to use here! Available interpolations are shown below
#define CURRENT_INTERPOLATION   (NO_INTERPOLATION)

static enum RESAMPLING_LIST
{
	NO_INTERPOLATION = 0, // default for PxTone
	LINEAR           = 1,
	CUBIC            = 2,
	SINC             = 3,
};
		...
		if( p_w = p_u->_p_woice )
		{
			...
			for( s32 ch = 0; ch < _dst_ch_num; ch++ )
			{
				...
				for( s32 v = 0; v < p_w->_voice_num; v++ )
				{
					...
					if( p_vt->life_count > 0 )
					{
						// resamples output for smooth audio
						b32 b_loop = ( p_w->_voices[ v ].voice_flags & PTV_VOICEFLAG_WAVELOOP );
						work += pxtnSample_interpolate( ch, p_vt->smp_pos, p_vi, CURRENT_INTERPOLATION, b_loop );

						if( _dst_ch_num == 1 )
						{
							work += pxtnSample_interpolate( ch ^ 1, p_vt->smp_pos, p_vi, CURRENT_INTERPOLATION, b_loop );
							work /= 2;
						}
