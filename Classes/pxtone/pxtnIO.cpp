...
{
	_EFFECT_STRAIN strn = {0};

	strn.group = (u16)p_strn->_group;
	strn.rate  = (f32)p_strn->_amp  ;

	// strn -----------
	s32 size = sizeof(strn) - sizeof(strn.gates);
	if( fwrite( &size, sizeof(s32), 1, fp ) != 1 ) return _false;
	if( fwrite( &strn, size,        1, fp ) != 1 ) return _false;
	return _true;
}
...
b32 pxtnIO_effeSTRANew_Write( FILE *fp, STRAIN_STRUCT *p_strn )
{
	...
	strn.group = (u16)p_strn->_group;
	strn.rate  = (f32)p_strn->_amp  ;
	for( s32 g = 0; g < GATE_NUM; g++ ) strn.gates[ g ] = (f32)p_strn->_gates[ g ];

	// strn -----------
	s32 size = sizeof(_EFFECT_STRAIN);
	if( fwrite( &size, sizeof(s32), 1, fp ) != 1 ) return _false;
	if( fwrite( &strn, size,        1, fp ) != 1 ) return _false;
	return _true;
}
...
b32 pxtnIO_effeSTRA_Read( DDV *p_read )
{
	...
	s32 old_size = sizeof(strn) - sizeof(strn.gates);
	u32 size;
	if( !ddv_Read( &size, sizeof(u32), 1, p_read )     ) return _false;
	if( !ddv_Read( &strn, old_size   , 1, p_read )     ) return _false;
	if( strn.xxx != 0                                  ) return _false;

	f32 cut = 100.0f - ( 100.0f / strn.rate ); // converts to modern overdrive value
	if( pxtnOverDrive_Add( cut, 1.0f, strn.group ) < 0 ) return _false;
	return _true;
}
...
b32 pxtnIO_effeSTRANew_Read( DDV *p_read, b32 *pbNew )
{
	...
	u32 size;
	if( !ddv_Read( &size, sizeof(u32           ), 1, p_read )   ) return _false;
	if( !ddv_Read( &strn, sizeof(_EFFECT_STRAIN), 1, p_read )   ) return _false;
	if( strn.xxx != 0                          ){ *pbNew = _true; return _false; }


	if( !pxtnStrain_Add( strn.group, (s32*)strn.gates, strn.rate ) ){ *pbNew = _true; return _false; }
	return _true;
}
